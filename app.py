...
def telegram_webhook():
    ...
    # --- Already a member, asking to onboard again ---
    onboard_words = ["onboard", "sign up", "signup", "join", "get started", "how do i start",
                     "want to join", "become a member"]
    if any(w in text for w in onboard_words) and not photos:
        known = member_for_chat(chat_id, username)
        if known and known.get("status") == "approved" and (known.get("access_code") or "").strip():
            reply(
                f"Good news, you've already got an account with us, so there's no need to onboard again.\n\n"
                f"If you've lost your login, here it is:\n\n"
                f"Your access code: {known['access_code']}\n"
    ...
    if photos and (is_payment_caption or _payment_pending.get(chat_id)):
        ...
        notify_admin(f"💷 ADVANCED COURSE PAYMENT\n\n{contact_block()}\n\n"
                     f"Tick to approve at https://innercircletrading.co/admin/photos\n\nScreenshot below:")
    ...
    if photos:
        ...
        if count == 1:
            ...
            notify_admin(
                ...
                f"then tick to approve at https://innercircletrading.co/admin/photos"
            )
            reply(
