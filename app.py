...
HER_ART_SVG = """<svg class="her-art" viewBox="0 0 1440 820" preserveAspectRatio="xMidYMid slice" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" focusable="false">
  <defs>
    <linearGradient id="herSky" x1="0" y1="0" x2="1" y2="1">
      <stop offset="0%" stop-color="#FBF5F3"/>
      <stop offset="48%" stop-color="#F6E9E5"/>
      <stop offset="100%" stop-color="#F0DCD7"/>
    </linearGradient>
    <linearGradient id="herFill" x1="0" y1="0" x2="0" y2="1">
      <stop offset="0%" stop-color="#C08B84" stop-opacity=".30"/>
      <stop offset="100%" stop-color="#C08B84" stop-opacity="0"/>
    </linearGradient>
    <radialGradient id="herGlow" cx="50%" cy="42%" r="60%">
      <stop offset="0%" stop-color="#FBF5F3" stop-opacity=".88"/>
      <stop offset="100%" stop-color="#FBF5F3" stop-opacity="0"/>
    </radialGradient>
  </defs>
  <rect width="1440" height="820" fill="url(#herSky)"/>
  <g stroke="#A9736B" stroke-width=".6" opacity=".13">
  ...
  <polyline points="0.0,571.5 12.1,566.6 24.2,562.5 36.3,556.5 48.4,550.1 60.5,547.0 72.6,544.0 84.7,535.3 96.8,530.2 108.9,525.1 121.0,514.8 133.1,507.8 145.2,498.3 157.3,491.1 169.4,482.8 181.5,477.7 193.6,469.5 205.7,459.9 217.8,452.6 229.9,444.1 242.0,436.3 254.1,432.6 266.2,424.6 278.3,418.1 290.4,413.7 302.5,411.4 314.6,403.9 326.7,399.4 338.8,393.6 350.9,387.1 363.0,382.0 375.1,375.9 387.2,373.6 399.3,368.9 411.4,366.9 423.5,361.9 435.6,357.5 447.7,358.4 459.8,359.3 471.9,359.8 484.0,355.5 496.1,354.7 508.2,352.7 520.3,352.9 532.4,351.7 544.5,351.1 556.6,350.7 568.7,348.6 580.8,346.3 592.9,341.7 605.0,338.3 617.1,333.1 629.2,328.0 641.3,321.7 653.4,317.2 665.5,315.6 677.6,309.2 689.7,301.7 701.8,294.3 713.9,288.8 726.1,281.9 738.2,278.1 750.3,269.9 762.4,263.1 774.5,257.9 786.6,254.0 798.7,244.7 810.8,234.4 822.9,229.8 835.0,220.5 847.1,213.8 859.2,208.7 871.3,202.7 883.4,193.6 895.5,184.0 907.6,180.0 919.7,172.4 931.8,168.7 943.9,160.9 956.0,155.9 968.1,147.5 980.2,138.8 992.3,133.5 1004.4,125.4 1016.5,122.1 1028.6,120.6 1040.7,116.1 1052.8,115.7 1064.9,114.5 1077.0,112.2 1089.1,108.9 1101.2,108.9 1113.3,109.9 1125.4,105.6 1137.5,105.3 1149.6,100.8 1161.7,96.9 1173.8,96.5 1185.9,95.6 1198.0,94.3 1210.1,92.2 1222.2,90.3 1234.3,88.6 1246.4,86.4 1258.5,81.8 1270.6,80.0 1282.7,78.4 1294.8,74.6 1306.9,73.7 1319.0,72.2 1331.1,65.8 1343.2,62.1 1355.3,57.9 1367.4,56.9 1379.5,52.9 1391.6,47.4 1403.7,45.3 1415.8,41.0 1427.9,41.0 1440.0,41.0" fill="none" stroke="#B5766E" stroke-width="2.6" opacity=".85" stroke-linejoin="round"/>
...
.her-hero-dark {
  ...
  border-bottom: 1px solid rgba(58,34,41,.12);
}
...
.her-hero-dark h1 {
  ...
  color: #3A2229; margin: 24px 0 20px; text-wrap: balance;
}
.her-hero-dark h1 em { font-style: italic; color: #B5766E; }
.her-hero-dark .lede {
  font-size: 17.5px; line-height: 1.75; color: #7E5F66;
  max-width: 560px; margin: 0 auto;
}
.her-tag {
  color: #A9736B !important;
  border-color: rgba(169,115,107,.4) !important;
...
.her-shout {
  display: block; text-decoration: none; color: inherit;
  grid-column: 1 / -1;
  background: linear-gradient(135deg, #B5766E 0%, #8E5A58 100%);
...
.plum .eyebrow { color: #A9736B !important; }
.plum h1, .plum h2, .plum h3, .plum h4 { color: #3A2229; }
.plum p, .plum li { color: var(--ink-dim); }
.plum .btn-primary {
  background: #3A2229; border-color: #3A2229; color: #FBF5F3;
}
.plum .btn-primary:hover { background: #55323C; border-color: #55323C; }
.plum .btn-ghost { border-color: rgba(58,34,41,.28); color: #3A2229; }
.plum .btn-ghost:hover { border-color: #A9736B; color: #A9736B; }
.plum .inline-link { color: #A9736B; }
.plum h1 em, .plum h2 em { color: #B5766E; }
.plum .topnav {
  background: rgba(251,245,243,.9);
  border-bottom: 1px solid rgba(58,34,41,.12);
}
.plum .brand { color: #3A2229; }
.plum .tabbar { background: #3A2229; }
.plum .tabbar a.active { color: #E9C7C0; border-bottom-color: #E9C7C0; }
.plum footer { background: #F4E7E3; }
.plum .benefit, .plum .form-panel, .plum .her-card, .plum .callout, .plum .hub-card {
  background: #FFFFFF; border-color: rgba(58,34,41,.10);
}
.plum .hub-bubble { background: #FFFFFF; border-color: rgba(58,34,41,.10); }
.plum .hub-bubble.is-team { background: #F4E7E3; border-color: #C08B84; }
.plum input, .plum select, .plum textarea {
  background: #FFFFFF !important; color: #3A2229 !important;
  border-color: rgba(58,34,41,.16) !important;
}
.plum input::placeholder, .plum textarea::placeholder { color: #B09098; }
/* the reading bubble is already paper, so it needs no change here */
