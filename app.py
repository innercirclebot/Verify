...
<div class="member-bar">
  <div class="wrap">
    ...
    <a href="/my-signals">Your Signals</a>
...
<div class="tabbar">
  <div class="wrap">
    <a href="/onboarding" class="{nav_class('onboarding')}">Onboarding</a>
    <a href="/education/fundamentals" class="{nav_class('fundamentals')}">Foundation Course</a>
    <a href="/education/advanced" class="{nav_class('advanced')}">Advanced Course</a>
    <a href="/community" class="{nav_class('community')}">Community</a>
    <a href="/signals" class="{nav_class('signals')}">Extra Signals</a>
  </div>
</div>
{member_bar}
{content}
    ...
    <div class="adm-grid">
      ...
      <div>
        <div class="adm-panel">
          ...
          <div class="adm-field"><span>Broker account</span><strong>{esc(member.get('account_number') or 'not set')}</strong></div>
          <div class="adm-field"><span>Deposit</span><strong>{esc(member.get('deposit_amount') or 'not set')}</strong></div>
...
def admin_member_access(member_id):
    ...
    if granted:
        member = get_member_by_id(member_id)
        if member and not (member.get("access_code") or "").strip():
            conn = get_db()
            if conn:
                ...
                try:
                    with conn, conn.cursor() as cur:
                        cur.execute("""UPDATE members SET access_code=%s, status='approved',
                                       ...
                                       WHERE id=%s""", (new_code, member_id))
                finally:
                    conn.close()
                audit(member_id, "access code issued", "created automatically when access was given")
                code_note = f" Access code {new_code} was created for them."
    ...
    if request.form.get("notify"):
        ...
        if member and member.get("chat_id"):
            ...
            fresh = get_member_by_id(member_id) or member
            if granted and fresh.get("access_code"):
                lines.append(f"\nLog in at https://innercircletrading.co/unlock with your code: {fresh['access_code']}")
                lines.append("All your group links live at https://innercircletrading.co/my-signals")
