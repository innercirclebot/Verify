    ...
    try:
        with conn, conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
            ...
            cur.execute("""UPDATE members SET community_approved=TRUE, community_requested=FALSE,
                           status='approved' WHERE id=%s""", (member_id,))
            cur.execute("SELECT * FROM members WHERE id=%s", (member_id,))
            return cur.fetchone()
    finally:
        conn.close()
...
"""
    ...
    comm_reqs = get_community_requests()
    comm_html = "".join(
        f'<div class="benefit" style="text-align:left; border-color:var(--rose);">'
        f'<strong>{m.get("title","")} {m.get("name","")}</strong> · {m.get("tier","")}'
        f'<br><span style="color:var(--ink-dim); font-size:12px;">Telegram: {m.get("telegram_username") or "none"} · '
        f'Existing code: {m.get("access_code") or "none yet"}</span>'
        f'<div style="margin-top:10px;"><a href="/admin/messages/{m["id"]}" class="inline-link" style="font-size:13px;">💬 Messages</a></div>'
        f'<form method="POST" action="/admin/grant-community/{m["id"]}" style="margin-top:10px;">'
        f'<button type="submit" class="btn btn-primary" style="padding:8px 18px; font-size:13px;">Approve Female Wealth</button></form>'
        f'</div>'
        for m in comm_reqs
    ) or "<p style='color: var(--ink-dim);'>No community requests.</p>"
    unread_banner = (
...
<section style="padding: 60px 0;">
  <div class="wrap">
    ...
    <h2 style="font-size: 20px; margin-bottom: 16px; color: var(--rose);">Female Wealth requests ({len(comm_reqs)})</h2>
    <div class="grid5" style="grid-template-columns: 1fr; margin-bottom: 48px;">{comm_html}</div>
...
def community_request():
    ...
    # If they're already logged in, flag THEIR existing account rather than making a new one
    existing_id = session.get("member_id")
    if existing_id:
        conn = get_db()
        if conn:
            try:
                with conn, conn.cursor() as cur:
                    cur.execute(
                        "UPDATE members SET community_requested=TRUE, title=COALESCE(NULLIF(%s,''), title), "
                        "telegram_username=COALESCE(NULLIF(%s,''), telegram_username) WHERE id=%s",
                        (title, tg, existing_id))
            finally:
                conn.close()
        member_id = existing_id
        linked_note = "LINKED to their existing account, approving unlocks it on their current code."
    else:
        member_id = create_pending_member(
            tier="community", title=title, name=name, account_number="",
            deposit_amount="", phone="", telegram_username=tg, referred_by=note
        )
        linked_note = "NOT logged in when requesting, so this is a separate record. They'll get their own code."
