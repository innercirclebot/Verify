...
def create_hub_post(member_id, title, body):
    ...
    try:
        with conn, conn.cursor() as cur:
            cur.execute("""INSERT INTO hub_posts (member_id, title, body)
                           VALUES (%s,%s,%s) RETURNING id""", (member_id, title, body))
            return cur.fetchone()[0]
    except Exception:
        return None
    finally:
...
def get_hub_posts(limit=60, include_hidden=False):
    conn = get_db()
    if not conn:
        return []
    try:
        with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
            cur.execute(f"""
                ...
                {"" if include_hidden else "WHERE p.hidden = FALSE"}
...
.hub-card:hover { border-color: var(--gold); border-left-color: var(--gold); transform: translateX(2px); }
...
.hub-bubble.is-team { background: var(--bg); border-color: var(--gold); }
...
def base_layout(title: str, content: str, active: str = "") -> str:
    ...
    if logged_in:
        ...
        links.append(("/hub", "Community Hub", "hub"))
...
@app.route("/hub")
def hub():
    """The question board. Open to any approved member."""
    if not is_verified():
        content = """
<section style="padding: 90px 0;">
  <div class="wrap" style="max-width: 620px; text-align: center;">
    ...
    <h1 style="font-size: 32px; margin: 12px 0 18px;">The Community Hub</h1>
    <p style="color: var(--ink-dim); font-size: 16px; margin-bottom: 30px;">
      Ask a question, get a proper answer, and keep it where you can find it again.
      In a group chat a good question scrolls away in an hour. Here it doesn't.
    </p>
    <div class="cta-row" style="justify-content: center;">
      <a href="/unlock" class="btn btn-primary">Log in</a>
      <a href="/onboarding" class="btn btn-ghost">Not a member yet</a>
    </div>
  </div>
</section>
"""
        return render_template_string(base_layout("Community Hub", content, "hub"))
    ...
    if posts:
        rows = "".join(
            ...
            + f'</p>'
            f'<div class="hub-meta">{esc(hub_display_name({"name": p.get("author_name")}))} · '
            f'{time_ago(p.get("last_activity") or p.get("created_at"))} · '
            f'{p.get("reply_count") or 0} '
            f'{"reply" if (p.get("reply_count") or 0) == 1 else "replies"}</div>'
            f'</a>'
            for p in posts
        )
    else:
        rows = ('<div class="callout">No questions yet. Ask the first one, there are no daft '
                'questions here and someone else is almost certainly wondering the same thing.</div>')

    content = f"""
<section style="padding: 48px 0 20px;">
  <div class="wrap" style="max-width: 760px;">
    <span class="eyebrow">Community Hub</span>
    <h1 style="font-size: 34px; margin: 10px 0 12px;">Ask anything.</h1>
    <p style="color: var(--ink-dim); font-size: 16px; max-width: 560px;">
      Questions and answers that stay put. Ask about a setup, a lesson, your platform,
      anything at all. The team answers here too.
    </p>
  </div>
</section>
...
<section style="padding: 10px 0 70px;">
  <div class="wrap" style="max-width: 760px;">
    <div class="form-panel" style="max-width: 100%; margin-bottom: 34px;">
      <h3 style="font-size: 18px; margin-bottom: 6px;">Ask a question</h3>
      ...
      <form method="POST" action="/hub/new">
        ...
        <input type="text" name="title" maxlength="160" required
               placeholder="What does a trailing stop actually do?">
        ...
        <button type="submit" style="margin-top: 16px;">Post question</button>
      </form>
    </div>
...
def hub_new():
    ...
    post_id = create_hub_post(member_id, request.form.get("title"), request.form.get("body"))
    if not post_id:
        return redirect("/hub?err=" + quote("Add a question and a bit of detail, then post it."))
    member = get_member_by_id(member_id)
    notify_admin(f"💬 NEW HUB QUESTION\n\n"
                 f"From: {member.get('name') or 'Member'} (#{member_id})\n"
                 f"{esc(request.form.get('title',''))[:160]}\n\n"
...
def hub_post(post_id):
    ...
    def bubble(name, when, body, from_team, item_id=None, kind="reply"):
        ...
        return (f'<div class="hub-bubble{" is-team" if from_team else ""}">'
                ...
                f'<strong style="color:var(--ink);">{esc(name)}</strong>{badge} · {time_ago(when)}{tools}</div>'
...
<section style="padding: 40px 0 70px;">
  <div class="wrap" style="max-width: 720px;">
    <a href="/hub" class="inline-link" style="font-size: 13px;">← All questions</a>
...
"""
    return render_template_string(base_layout(post["title"], content, "hub"))
...
def hub_reply(post_id):
    ...
    if not add_hub_reply(post_id, member_id, request.form.get("body"), from_team):
        return redirect(f"/hub/{post_id}?err=" + quote("Write something first."))
    if not from_team:
        notify_admin(f"💬 NEW HUB REPLY\n\nOn: {SITE}/hub/{post_id}")
    return redirect(f"/hub/{post_id}")
...
<section id="support">
  <div class="wrap" style="max-width: 820px;">
    <div class="section-head" style="max-width:100%; text-align:center;">
      ...
      <p>Stuck on a step, or not sure what's next? Ask us. You'll always get an answer, never left to figure it out yourself. Members can also ask in the Community Hub, where questions get answered properly and stay put instead of scrolling away in a group chat.</p>
    </div>
    <div class="cta-row" style="justify-content:center;">
