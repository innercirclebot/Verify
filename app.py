...
def her_library():
    """Everything the masterclasses point at, in one place."""
    if not has_access("her") and not session.get("admin"):
    ...
    for area, blurb, books in HER_LIBRARY:
        items = "".join(tile(t, a, w) for t, a, w in books)
    ...
    audio = "".join(tile(t, a, w) for t, a, w in HER_AUDIO)
...
<section style="padding:52px 0 70px;">
  <div class="wrap" style="max-width:760px;">
    ...
    <div class="section-head" style="max-width:100%; margin:22px 0 34px;">
      ...
      <h1 style="font-size:34px; margin:10px 0 12px;">Read this, listen to this</h1>
    ...
    <div style="margin-bottom:20px;">
      <h2 class="lib-h">Music &amp; audio</h2>
      <p class="lib-p">Put these on while you get ready, not in the background.
...
"""
    return render_template_string(base_layout("Library", content, "community", theme="plum"))
...
def her():
    ...
    # the sticker clears itself the moment a video exists
...
<section style="padding-top:60px;">
  <div class="wrap" style="max-width:680px;">
    ...
    <a href="/her/events" class="her-card">
      ...
      <span class="her-title">Get togethers<br><span style="font-family:'Inter'; font-size:13px; color:var(--ink-dim);">What's coming up, and whether you're in</span></span>
      <span class="her-arrow">→</span>
    </a>

    <a href="/her/library" class="her-card">
      <span class="her-num">05</span>
      <span class="her-title">Read this, listen to this<br><span style="font-family:'Inter'; font-size:13px; color:var(--ink-dim);">Books, audio and the music that goes with the work</span></span>
      <span class="her-arrow">&#8594;</span>
    </a>
...
def her_start():
    ...
    next_up = next((t for t in path if t not in done), None)
    done_count = sum(1 for t in path if t in done)
    ...
    if next_up:
        hero = (
            ...
            f'{done_count} of {len(path)} done. One lesson at a time is the whole method.</p>'
        ...
        hero = ('<div class="her-panel" style="margin-bottom:26px;">'
                '<h3 style="font-size:22px; margin:0 0 10px;">Every lesson, done.</h3>'
                '<p style="color:var(--ink-dim); font-size:14px; margin:0;">All of it. '
                'Go back to the ones that landed hardest, they read differently the second time.</p></div>')
...
def her_course(slug):
    ...
    for cat_name, cat_desc, titles in HER_CATEGORIES:
        ...
        done_here = sum(1 for t in real if t in done)
        count_html = (f'<span style="color:var(--rose); font-size:13px; font-weight:600;"> · '
                      f'{done_here} of {len(real)} done</span>') if done_here else ""
        items = "".join(
            f'<a href="/her/masterclass/{title_to_idx[t]}" class="her-card small">'
            f'<span class="her-title">'
            + ('<span style="color:var(--rose); font-weight:700;">&#10003;&nbsp;</span>' if t in done else '')
            + f'{esc(t)}</span><span class="her-arrow">→</span></a>'
            for t in real)
        blocks.append(
