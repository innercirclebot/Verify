...
def create_hub_post(member_id, title, body, space="main"):
...
def get_hub_posts(limit=60, include_hidden=False, space="main"):
    conn = get_db()
    if not conn:
        return []
    try:
...
<section style="padding: 48px 0 70px;">
  <div class="wrap">
    ...
    <div class="adm-bar">
      ...
      <a href="/hub" class="">Community Hub</a>
      <a href="/her/hub" class="">Female Wealth board</a>
...
SPACES = {
    "main": {
        ...
        "title": "Ask anything.",
        "blurb": ("Questions and answers that stay put. Ask about a setup, a lesson, your "
                  "platform, anything at all. The team answers here too."),
        "prompt": "What does a trailing stop actually do?",
        "empty": ("No questions yet. Ask the first one, there are no daft questions here and "
                  "someone else is almost certainly wondering the same thing."),
        "cta": "Ask a question",
        "post_label": "Post question",
    },
    "her": {
        "path": "/her/hub",
        "nav": "community",
...
def admin_hub():
    ...
    space = request.args.get("space", "main")
    ...
    if posts:
        ...
        for p in posts:
            ...
            rows.append(
                f'<div class="adm-panel" style="margin-bottom:14px;">'
                f'<div class="adm-card-top">'
                f'<a href="/hub/{p["id"]}" class="adm-card-name" style="text-decoration:none;">'
                f'{esc(p["title"])}</a></div>'
                ...
                f'<a href="/hub/{p["id"]}" class="btn btn-ghost btn-sm">Open thread</a>'
...
<section style="padding: 48px 0 70px;">
  <div class="wrap" style="max-width: 800px;">
    ...
    <div class="adm-bar">
      <a href="/admin/hub?space=main" class="{'on' if space == 'main' else ''}">Community Hub</a>
      <a href="/admin/hub?space=her" class="{'on' if space == 'her' else ''}">Female Wealth board</a>
    </div>
...
"""
    return render_template_string(base_layout("Board moderation", content, ""))
...
def admin_hub_action(post_id, action):
    ...
    space = request.form.get("space", "main")
    if space not in SPACES:
        space = "main"
...
def hub_post(post_id):
    ...
    space = (post.get("space") or "main")
    if space == "her" and not (has_access("her") or is_admin):
        return redirect("/community")
    back = SPACES.get(space, SPACES["main"])
...
def hub_reply(post_id):
    ...
    if post and (post.get("space") or "main") == "her" and not (has_access("her") or from_team):
        return redirect("/community")
