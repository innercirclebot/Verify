...
def upsert_photo_submission(chat_id, username, first_name, kind, count):
    conn = get_db()
    if not conn:
        return
    try:
        with conn, conn.cursor() as cur:
            ...
            row = cur.fetchone()
            if row:
                cur.execute("UPDATE photo_submissions SET photo_count=%s, created_at=NOW() WHERE id=%s",
                            (count, row[0]))
            else:
                cur.execute("""INSERT INTO photo_submissions (chat_id, username, first_name, kind, photo_count)
                               VALUES (%s,%s,%s,%s,%s)""", (chat_id, username, first_name, kind, count))
    finally:
        conn.close()
...
def get_photo_submissions():
    ...
    try:
        with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
            cur.execute("SELECT * FROM photo_submissions WHERE status='pending' ORDER BY created_at DESC")
            return cur.fetchall()
    finally:
        conn.close()
        ...
        with conn, conn.cursor() as cur:
            ...
            cur.execute("""
                INSERT INTO members (tier, title, name, account_number, deposit_amount, phone,
                                      phone_normalized, telegram_username, verification_code,
                                      referred_by, chat_id)
    ...
    try:
        with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
            if search:
                ...
                """, (like, like, like, like, like, like, norm or "", norm or "", limit))
            else:
...
MERGE_FIELDS = ["title", "name", "account_number", "deposit_amount", "phone",
...
def update_member_details(member_id, fields):
    ...
    allowed = ["title", "name", "phone", "account_number", "deposit_amount",
...
def member_card(m, sections):
    """One clickable row in the admin member lists."""
    ...
    if m.get("community_requested") and "her" not in sections:
        pills += '<span class="pill gold">Circle request waiting</span>'

    tg = (m.get("telegram_username") or "").lstrip("@")
    meta = " · ".join(filter(None, [
        (m.get("tier") or "").title(),
        f"Acct {esc(m['account_number'])}" if m.get("account_number") else "",
        f"£{esc(m['deposit_amount'])}" if m.get("deposit_amount") else "",
        f"@{esc(tg)}" if tg else "no Telegram",
        "chat linked" if m.get("chat_id") else "no chat linked",
        f"joined {fmt_date(m.get('created_at'))}",
    ]))

    return (
        f'<a class="adm-card {cls}" href="/admin/member/{m["id"]}">'
...
"""
    ...
    if unread:
        unread_banner = (
            ...
            f'Open a profile to read and reply.</div>'
        )

    cards = "".join(member_card(m, sections_by_member.get(m["id"], set())) for m in members)
    if not cards:
        empty = {
...
<section style="padding: 48px 0 70px;">
  <div class="wrap">
    ...
    <div class="adm-panel">
      <h3>Resend someone's access code</h3>
      <p class="hint">For members who've lost the code they were sent at onboarding. Look them up by phone number, name or Telegram handle. Their code stays the same, so anything they'd already written down still works.</p>
      <form method="POST" action="/admin/resend-code" class="adm-search" style="margin: 0;">
        <input type="text" name="lookup" placeholder="Phone number, name or @username" required>
        <button type="submit">Find and resend</button>
      </form>
    </div>

    <div class="adm-panel" style="margin-bottom: 30px;">
      <h3>Send a verification code</h3>
      <p class="hint">After checking someone's onboarding screenshots, send them their code. Enter their @username or chat ID.</p>
      <form method="POST" action="/admin/send-code" class="adm-search" style="margin: 0;">
        <input type="text" name="target" placeholder="@username or chat ID">
...
def admin_member(member_id):
    ...
    if member.get("absorbed"):
        ...
        absorbed_panel = f"""
    <div class="adm-panel">
      <h3>Merged into this profile</h3>
      <p class="hint">These records were folded in here. Separating one puts it back in the waiting list on its own.</p>
      {rows}
    </div>"""
    ...
    <div class="adm-grid">
        ...
        <div class="adm-panel">
          ...
          <form method="POST" action="/admin/member/{member_id}/access">
            ...
            <label class="tick" style="border-style: dashed;">
              ...
              <span><b>Tell them on Telegram</b><small>Sends a short note listing what changed</small></span>
            </label>
      ...
      <div>
        <div class="adm-panel">
          ...
          <div class="adm-field"><span>Tier</span><strong>{esc(member.get('tier') or '—')}</strong></div>
          <div class="adm-field"><span>Status</span><strong>{esc(member.get('status') or '—')}</strong></div>
          <div class="adm-field"><span>Broker account</span><strong>{esc(member.get('account_number') or '—')}</strong></div>
          <div class="adm-field"><span>Deposit</span><strong>{esc(member.get('deposit_amount') or '—')}</strong></div>
          ...
          <div class="adm-field"><span>Verification code</span><strong>{esc(member.get('verification_code') or '—')}</strong></div>
          <div class="adm-field"><span>Referred by</span><strong>{esc(member.get('referred_by') or '—')}</strong></div>
          <div class="adm-field"><span>Approved</span><strong>{fmt_date(member.get('approved_at')) or '—'}</strong></div>
          <div class="adm-field"><span>Messages</span><strong><a href="/admin/messages/{member_id}" class="inline-link">{member['message_count']} in thread</a> {unread_pill}</strong></div>
        </div>
...
def admin_member_access(member_id):
    ...
    if request.form.get("notify"):
        ...
        if member and member.get("chat_id"):
            lines = []
            if granted:
                lines.append("You've been given access to: " + ", ".join(SECTION_LABELS[s] for s in granted))
            if revoked:
                lines.append("Access removed for: " + ", ".join(SECTION_LABELS[s] for s in revoked))
            if granted and member.get("access_code"):
                lines.append(f"\nLog in at https://innercircletrading.co/unlock with your code: {member['access_code']}")
            send_telegram_message(member["chat_id"], "Your Inner Circle access has been updated.\n\n" + "\n".join(lines))
    ...
    return redirect(f"/admin/member/{member_id}?ok=" + quote("Access saved: " + "; ".join(bits)))
...
<section style="padding: 48px 0 70px;">
  <div class="wrap">
    ...
    <p style="color: var(--ink-dim); margin-bottom: 26px;">Grouped by phone number, since that's what identifies a member.</p>
    {flash_banner()}
    {body}
  </div>
</section>
...
def community_request():
    ...
    if member:
            ...
            try:
                with conn, conn.cursor() as cur:
                    cur.execute(
                        ...
                        (title, name, phone, normalize_phone(phone), member_id))
            finally:
                conn.close()
        ...
        member_id = create_pending_member(
            ...
            deposit_amount="", phone=phone, referred_by=note
        )
        conn = get_db()
        if conn:
            try:
