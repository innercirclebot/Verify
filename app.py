...
def admin_decline_requested(member_id):
    """
    The other button. A new onboarder is told to check their details and photos
    and resubmit, or to message us for help. An existing member asking for more
    gets the admin's own words on why, since they already have an account. The
    message goes to their Telegram when a chat is linked, and always onto their
    site thread so there's a record either way.
    """
    if not session.get("admin"):
        return redirect(url_for("admin"))
    ...
    back = request.form.get("back") or "/admin"
    joiner = "&" if "?" in back else "?"
    return redirect(f"{back}{joiner}ok=" +
                    quote(f"Declined {member.get('name') or 'member'}. {how}"))
...
def handle_admin_reply(message) -> bool:
    ...
    if not replied:
        return False
    word = (message.get("text") or "").strip().lower().rstrip("!.")
    approving = word in APPROVE_WORDS
    rejecting = word in REJECT_WORDS
    ...
    if action == "verify":
        ...
        if rejecting:
            ...
            clear_tg_action(replied.get("message_id"))
            say("Noted, marked as not approved. Nothing has been sent to them, so message "
                f"them from their chat on the site if you want to explain: {SITE}/admin/photos")
            return True
    ...
    if action == "member":
        ...
        if rejecting:
            clear_tg_action(replied.get("message_id"))
            say(f"Left alone, nothing granted and nothing sent. Their profile: "
                f"{SITE}/admin/member/{ref}")
            return True
    ...
    if action == "sub":
        ...
        if rejecting:
            resolve_photo_submission(ref, "rejected")
            clear_tg_action(replied.get("message_id"))
            say("Noted, marked as not approved and nothing unlocked.")
            return True
        member_id = sub.get("member_id")
        if not member_id:
