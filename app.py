...
def init_db():
    ...
    try:
        with conn, conn.cursor() as cur:
            ...
            for col, coltype in [("member_id", "INT"),
                                 ("message_id", "BIGINT"),
...
"""
    ...
    if search:
        ...
        heading = f"Wealth Circle requests ({len(members)})"
    else:
        view = "pending"
    ...
    if not cards:
        empty = {
            ...
            "community": "No Wealth Circle requests right now.",
            "all": "No members yet.",
...
<section style="padding: 48px 0 70px;">
  <div class="wrap">
    ...
    <div class="adm-stats">
      ...
      <div class="adm-stat"><b>{counts['community']}</b><span>Circle requests</span></div>
    ...
    <div class="adm-bar">
      ...
      <a href="/admin?view=community" class="{'on' if view == 'community' else ''}">Circle requests ({counts['community']})</a>
...
def admin_photos():
    ...
    for sub in subs:
        ...
        rows.append(
            ...
            f'{checklist}{warn}'
            f'<p class="hint" style="margin:12px 0 10px;">The screenshots were forwarded to your '
            f'Telegram admin chat. Check them there, then tick here.</p>'
...
def set_bot_state(chat_id, state=None, photo_count=None, greeted=None):
    ...
    try:
        with conn, conn.cursor() as cur:
            cur.execute("""INSERT INTO bot_state (chat_id, state, photo_count, greeted, updated_at)
                           VALUES (%s,%s,%s,%s,NOW())
                           ON CONFLICT (chat_id) DO UPDATE SET
                             ...
                             updated_at = NOW()""",
                        (chat_id, state, photo_count or 0, bool(greeted),
                         state, photo_count, greeted))
    except Exception:
        pass
    finally:
        conn.close()
...
def clear_bot_state(chat_id):
    ...
    try:
        with conn, conn.cursor() as cur:
            cur.execute("""UPDATE bot_state SET state=NULL, photo_count=0, updated_at=NOW()
...
def telegram_webhook():
    ...
    def build_menu(intro):
        """
        The menu doubles as the help text, so it lists the exact words that work
        and shows people where each one lives on the site.
        """
        lines = [intro, "", "Message me any of these words:", ""]
        if is_member:
            lines += [
                "🔑  \"lost code\"  -  your login code, sent straight back",
                "🔗  \"lost links\"  -  every group you have access to",
                "📚  \"course\"  -  your Trading Fundamentals lessons",
                "📈  \"advanced\"  -  the £99 chart reading course",
                "➕  \"extra signals\"  -  more groups and currency pairs",
                "👭  \"community\"  -  Wealth Circle, our women-only group",
                "🎉  \"share results\"  -  send us your wins",
                "",
                "Everything you have access to is also on your account:",
                "https://innercircletrading.co/account",
                "All your group links: https://innercircletrading.co/my-signals",
            ]
        else:
            lines += [
                "🚀  \"onboarding\"  -  how to get set up and on the signals",
                "✅  \"activate\"  -  once you've done the onboarding steps",
                "📚  \"course\"  -  our free 41 lesson beginners course",
                "📈  \"advanced\"  -  chart reading course, £99 one-time",
                "➕  \"extra signals\"  -  more groups and currency pairs",
                "👭  \"community\"  -  Wealth Circle, our women-only group",
                "🔑  \"lost code\"  -  if you can't log in",
                "",
                "New here? Start at https://innercircletrading.co/onboarding",
            ]
        lines += [
            "",
            "You can also just ask me a question in your own words, about deposits, MT5, "
            "lot sizes, anything. If I can't answer it I'll pass it to a real person.",
            "",
            "Stuck? Say \"help\" any time to see this again.",
        ]
        return "\n".join(lines)
    ...
    if state == "awaiting_photo_purpose" and not photos:
        ...
        if any(w in text for w in ("payment", "paid", "advanced", "course", "2", "two")):
            ...
            reply("Got it, that's your Advanced course payment. Send the payment screenshot over "
                  "and I'll pass it straight to our admin team to unlock your access.")
            return "ok"
    ...
    if photos:
        ...
        # A photo out of the blue. Ask rather than guess.
        set_bot_state(chat_id, state="awaiting_photo_purpose")
        forward_photo_to_admin(chat_id, msg_id)
        notify_admin(f"📷 PHOTO, PURPOSE UNKNOWN\n\n{contact_block()}\n\n"
                     f"I've asked them what it's for. Image below.")
    ...
    # -----------------------------------------------------------------
    onboard_words = ["onboard", "sign up", "signup", "get started", "how do i start",
                     "want to join", "become a member", "how do i join", "getting started"]
    if any(w in text for w in onboard_words):
        if is_member:
            reply(f"Good news, you've already got an account with us, so there's no need to "
                  f"onboard again.\n\n"
                  f"If you've lost your login, here it is:\n\n"
                  f"Your access code: {member['access_code']}\n"
                  f"Log in at https://innercircletrading.co/unlock\n\n"
                  f"All of your group links are saved here too:\n"
                  f"https://innercircletrading.co/my-signals\n\n"
                  f"Anything else you need, just ask.")
            return "ok"
        set_bot_state(chat_id, state="awaiting_activation", greeted=True)
        reply(ONBOARDING_STEPS)
        return "ok"

    activate_words = ["activate", "activated", "activation done", "done the steps", "completed the steps",
                      "finished onboarding", "done onboarding", "ive done it", "i've done it",
                      "all done", "completed"]
    if any(w in text for w in activate_words):
        if is_member:
            reply("You're already activated and on the signals. If something's not working, "
    ...
    # -----------------------------------------------------------------
    # 5. Advanced course and payment
    # -----------------------------------------------------------------
    PAID_EXACT = ("paid", "i paid", "ive paid", "i've paid", "paid it", "payment done",
                  "paid now", "i have paid", "payment sent", "sent payment")
    said_paid = (text in PAID_EXACT
                 or any(w in text for w in payment_words)
                 or (state == "offered_advanced" and text in ("yes", "yep", "done", "ok", "okay")))
    if said_paid:
        set_bot_state(chat_id, state="awaiting_payment_photo")
        reply("Great, thanks for letting me know.\n\n"
              ...
              "I'll message you here.")
        return "ok"

    advanced_words = ["advanced", "chart reading", "paid course", "99", "technical analysis course"]
    if any(w in text for w in advanced_words):
        if member and "advanced" in get_member_sections(member["id"]):
            reply("You've already got Advanced Chart Reading unlocked. Open it here:\n"
                  "https://innercircletrading.co/education/advanced\n\n"
                  "Log in with your access code if it asks.")
            return "ok"
        set_bot_state(chat_id, state="offered_advanced")
        reply(ADVANCED_STEPS)
        return "ok"

    # -----------------------------------------------------------------
    # 6. Lost code and lost links
    # -----------------------------------------------------------------
    lost_code_words = ["lost my code", "lost code", "lost my password", "lost password",
                       "forgot my code", "forgot code", "forgot my password", "forgotten my code",
                       "can't log in", "cant log in", "cannot log in", "can't login", "cant login",
                       "access code", "my code", "log in details", "login details"]
    if any(w in text for w in lost_code_words):
        if is_member:
            reply(f"No problem, here it is.\n\n"
                  f"Your access code: {member['access_code']}\n"
                  f"Log in at https://innercircletrading.co/unlock\n\n"
                  f"It'll keep you signed in on that device, so you shouldn't need it often. "
                  f"Worth saving it somewhere though.")
            audit(member["id"], "code resent by bot", "they asked for it on Telegram")
        elif member:
            reply("I can see your account but it hasn't been approved yet, so there's no code on "
                  "it so far. Once your screenshots are checked I'll send it straight here.")
        else:
            reply("I can't match this Telegram chat to an account, which usually means you signed "
                  "up with a different one.\n\n"
                  "Head to https://innercircletrading.co/unlock and if you're still stuck, message "
                  "us on the site at https://innercircletrading.co/messages and we'll get you back in.")
        return "ok"

    lost_links_words = ["lost the links", "lost links", "lost my links", "lost the link",
                        "left the group", "left group", "removed from the group", "group link",
                        "signals link", "can't find the group", "cant find the group",
                        "rejoin", "join the group again"]
    if any(w in text for w in lost_links_words):
        links = links_for_member()
        if links:
            reply(links)
        elif member:
            reply("Your account isn't approved yet, so there aren't any group links on it so far. "
                  "As soon as it's approved I'll send them straight here.")
        else:
            reply("I can't match this chat to an account, so I can't tell which groups are yours.\n\n"
                  "Log in at https://innercircletrading.co/unlock and every link you have is saved "
                  "on your account at https://innercircletrading.co/my-signals")
        return "ok"

    # -----------------------------------------------------------------
    # 7. Sharing results
    # -----------------------------------------------------------------
    if any(w in text for w in results_words):
        set_bot_state(chat_id, state="awaiting_results_photos")
        reply("Love it, send your screenshots over and I'll pass them straight to the team. "
              "Always great to see how everyone's getting on.")
        return "ok"

    # -----------------------------------------------------------------
    # 8. Anything else: known answers, then trading questions, then a human
    # -----------------------------------------------------------------
    if raw_text:
        ...
        TRADING_WORDS = ["lot size", "pip", "leverage", "stop loss", "take profit", "spread",
                         "candlestick", "support", "resistance", "risk management", "chart",
                         "indicator", "moving average", "rsi", "fibonacci", "trend", "entry",
                         "how do i trade", "how to trade", "what is a", "what's a", "margin",
                         "drawdown", "scalping", "swing trade", "analysis"]
        if any(w in text for w in TRADING_WORDS):
            if member and "fundamentals" in get_member_sections(member["id"]):
                reply("That's covered properly in our Trading Fundamentals course, which you've "
                      "already got:\n\nhttps://innercircletrading.co/education/fundamentals\n\n"
                      "It's free and it walks through everything step by step. If you're still "
                      "unsure after the relevant lesson, ask me again and I'll get you a proper answer.")
            else:
                reply("Good question, and it's one you'll cover properly in our Trading Fundamentals "
                      "course. It's free and you get it as soon as you're onboarded.\n\n"
                      "Message me \"onboarding\" and I'll walk you through getting set up.")
            return "ok"

        reply("Good question, I want to make sure you get the right answer on that one so I'm "
              "passing it to the team, they'll be in touch shortly.\n\n"
              "In the meantime, say \"help\" and I'll show you everything I can sort out for you "
              "right now.")
        notify_admin(f"❓ UNANSWERED MESSAGE\n\n{contact_block()}\n\nTheir message:\n{raw_text}\n\n"
                     f"Reply from the site: https://innercircletrading.co/admin/inbox")
